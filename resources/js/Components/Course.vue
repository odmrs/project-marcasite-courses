    ...
    <div class="w-full max-w-4xl">
        ...
        <form v-if="editing" @submit.prevent="form.put(route('courses.update', { course: course.id }), { onSuccess: () => editing = false })">
            ...
            <div class="mt-4">
                ...
                <input v-model="form.start_date" type="date" id="start_date" name="start_date" class="mt-1 p-2 block w-full text-gray-900 border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm" @change="updateEndDateMin">
                <InputError :start_date="form.errors.start_date" class="mt-2" />
            </div>
            <!-- end date  -->
            <div class="mt-4">
                <label for="end_date" class="block text-sm font-medium text-gray-700">End Date</label>
                <input v-model="form.end_date" type="date" id="end_date" name="end_date" class="mt-1 p-2 block w-full text-gray-900 border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm" :min="form.start_date">
